...
@login_required
def new_conversation(request:HttpRequest, item_pk):
    ...
    if conversations:
        redirect("conversation:detail", conversation_pk=conversations.first().id)
...
@login_required
def detail(request:HttpRequest, conversation_pk):
